...
contract ERC721 is IERC721, IERC721Metadata {
    using Strings for uint256; // String库
    ...
    function _transfer(
        ...
        require(from == owner, "not owner"); // 必须是 owner 才能转账
        require(to != address(0), "transfer to the zero address");
        _approve(owner, address(0), tokenId); // tokenId 授权给 0地址? 重置授权?

        _balances[from] -= 1;
        _balances[to] += 1;
        _owners[tokenId] = to; // NFT 不是 owner 以后,之前的授权应该重置吧?
    ...
    function tokenURI(
        ...
        // 计算{tokenURI}的BaseURI，tokenURI就是把baseURI和tokenId拼接在一起
        return
    ...
    function _burn(uint tokenId) internal virtual {
        address owner = ownerOf(tokenId);
        require(msg.sender == owner, "not owner of tokenId");
